...
有时候, 指定的堆内存大小, 不能满足程序正常运行的需求. 这时候只要增加堆内存的大小即可, 配置参数可以参考下文。
...
然而, 在许多情况下, 增加更大的堆内存空间, 也不能解决问题。例如, 程序中存在内存泄漏, 增加堆内存也只能推迟触发 _java.lang.OutOfMemoryError: Java heap space_ 错误的时间。此外,增加Java堆的大小也可能会增加 [GC pauses](https://plumbr.eu/handbook/gc-tuning/gc-tuning-in-practice/tuning-for-throughput) 的时间, 从而影响程序的 [吞吐量或延迟](https://plumbr.eu/handbook/gc-tuning/throughput-vs-latency-vs-capacity)。
...
如果你想从根本上解决这个问题, 则需要排查到底是哪些代码负责分配内存. 换句话说, 你需要回答这些问题:
...
在这一点上, 可能需要持续几天才能分析完。下面是一个大致的流程:
